...
- **Startup guardrails:** Increase `MIDDLEWARE_PLUGIN_TIMEOUT_MS` (default `60000`) when large datasets make schema checks slow so Fastify waits for the datastore plugin to finish instead of aborting early, and tune `DATASTORE_CONNECT_MAX_RETRIES` / `DATASTORE_CONNECT_RETRY_DELAY_MS` to keep retrying PostgreSQL handshakes while containers finish booting.
